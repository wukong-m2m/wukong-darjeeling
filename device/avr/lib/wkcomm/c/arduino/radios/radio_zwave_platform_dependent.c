...
#ifdef WUTINY
    static uint32_t zwave_press=0;
    if (zwave_press == 0) {
        if(ZWAVEPORT == 0) {
			zwave_press = dj_timer_getTimeMillis();
        ...
        if(ZWAVEPORT) {
			if (dj_timer_getTimeMillis() > zwave_press + 100) {
