...
void wkpf_comm_handle_message(void *data) {
	...
	switch (msg->command) {
		...
		case WKPF_COMM_CMD_CHANGE_MAP: {
			DEBUG_LOG(DBG_WKCOMM, "Received change component map request\n");
			...
			orig_node = (uint32_t)payload[2];
			orig_node = (uint32_t)(orig_node<<8) + (uint16_t)(payload[3]);
			orig_node = (uint32_t)(orig_node<<8) + (uint16_t)(payload[4]);
			orig_node = (uint32_t)(orig_node<<8) + (uint16_t)(payload[5]);
			orig_port = (uint16_t)(payload[6]);
			new_node = (uint32_t)(payload[7]);
			new_node = (uint32_t)(new_node<<8) + (uint16_t)(payload[8]);
			new_node = (uint32_t)(new_node<<8) + (uint16_t)(payload[9]);
			new_node = (uint32_t)(new_node<<8) + (uint16_t)(payload[10]);
			new_node = (uint32_t)(new_node<<8) + (uint16_t)(payload[11]);
			new_port = (uint16_t)(payload[12]);
			...
			if (retval != WKPF_OK) {
				...
				DEBUG_LOG(DBG_WKCOMM, "Successully change component map\n");
				payload[0] = retval;
				response_size = 1;
		...
		break;
		case WKPF_COMM_CMD_GET_LINK_COUNTER: {
			uint16_t link_id;
		...
		case WKPF_COMM_CMD_GET_DEVICE_STATUS: {
			DEBUG_LOG(DBG_WKCOMM, "Received get device status\n");
			payload[0] = WKPF_OK;
