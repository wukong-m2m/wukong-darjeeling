...
if len(db) == 0: print "None"
    ...
    for (node_id, node) in db.iteritems():
        if is_id_gateway and (node.gateway_id == to_del_node_id):
            del db[node_id]
            continue

        print "Node ID:", int(node_id), MPTN.ID_TO_STRING(int(node_id)), "IF_ADDR:", MPTN.ID_TO_STRING(node.if_address), "IS_GATEWAY:", node.is_gateway, "GATEWAY ID:", MPTN.ID_TO_STRING(node.gateway_id), "UUID:", map(ord, node.uuid)
...
if len(db) == 0: print "None"
    ...
    for (network_string, tcp_address) in db.iteritems():
        if is_id_gateway and MPTN.IS_ID_IN_NETWORK(to_del_node_id, MPTN.ID_NETWORK_FROM_STRING(network_string)):
            del db[network_string]
            continue
...
if len(db) == 0: print "None"
    ...
    for (address, uuid) in db.iteritems():
        if int(address) == to_del_node_id:
            ...
            print "Registered address and its UUID:", int(address), MPTN.ID_TO_STRING(int(address)), map(ord, uuid)
...
if to_del_node_id & 0xffffff00 != db["GTWSELF_ID"] & 0xffffff00:
    exit()
...
if CONFIG.TRANSPORT_INTERFACE_TYPE == 'udp':
    ...
    try:
        ...
        for i, d in enumerate(devices):
            print d.host_id, d.ip, MPTN.ID_TO_STRING(d.ip), d.port
