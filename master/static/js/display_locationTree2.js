...
function display_tree(container_id,rt) {
    ...
    */
    html_tree = '<p><b>Double Click a Tree Node to Start Editing</b> <button id="saveTree" class="my-button my-button-blue">Save Tree</button></p>';
    html_tree += '<div class="tree-editor-div"><table width="100%">';
    html_tree += '<tr><td width="5%"></td><td></td><td></td></tr>';
    html_tree += '<tr><td></td><td>';
    html_tree += '</td><td valign="top">';
    if (container_id == "#landmark_editor"){    //#content means in user mode, generating for landmark edit
    ...
    html_tree += '</td></tr></table></div>';
    $(container_id).append(html_tree);
    $("#display").treeview({
        collapsed: true,
        unique: true
    });
    $('.tree-editor-div').height($('.tree-editor-div').height()-100)
    $('.tree-editor-div').css('overflow-y','auto')
}
