...
let resultsToString (results : SimulationResults) =
    ...
    addLn (String.Format ("              C   total counters + timer {0,14} (={0,14}+{0,14})", results.countersCTotalPlusTimer.cycles, results.countersCTotal.cycles, results.countersCTimer.cycles))
    addLn (String.Format ("              C   ratio                  {0,14} (difference probably caused by interrupts)", (cyclesToSlowdown results.countersCTotalPlusTimer.cycles results.cyclesStopwatchC)))
    addLn ("")
