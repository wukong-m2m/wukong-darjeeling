...
void javax_rtcbench_NewSignature_void_initNewSignature_javax_rtcbench_NewSignature_short() {
	...
	// avroraPrintDJHeap();
	// avroraPrintPtr(dest_newsignature);
...
void javax_rtcbench_NewSignature_void_initRefSignature_javax_rtcbench_RefSignature_short() {
	int16_t index = dj_exec_stackPopShort();
	...
	for (uint8_t i=0; i<NBR_RFSIGNALS_IN_SIGNATURE; i++) {
		...
		avroraPrintHex32(refSig.sig.rfSignals[i].sourceID);
		dest_rfsignal->sourceID = refSig.sig.rfSignals[i].sourceID;
		avroraPrintHex32(dest_rfsignal->sourceID);
		avroraPrintPtr(&(dest_rfsignal->sourceID));
		avroraPrintPtr(&(dest_rfsignal->rssi));
...
void javax_rtcbench_NewSignature_void_initSignature_javax_rtcbench_Signature_short() {
	// int16_t index = dj_exec_stackPopShort();
	// BM_MOTETRACK_STRUCT_javax_rtcbench_Signature * dest_signature = (BM_MOTETRACK_STRUCT_javax_rtcbench_Signature *)REF_TO_VOIDP(dj_exec_stackPopRef());
}
