#include "execution.h"
#include "array.h"
#include "jlib_bm_motetrack_nca.h"
#include "RefSignature.h"
#include "SignatureDB.h"
#include "RefSignatureDB.h"

int16_t MoteTrack_DB_index;
BM_MOTETRACK_NCA_STRUCT_javax_rtcbench_RefSignature * MoteTrack_DB_dest_refsignature;
...
void javax___read_refsignature_from_flash() {
    ...
    BM_MOTETRACK_NCA_STRUCT_javax_rtcbench_Point * dest_location = (BM_MOTETRACK_NCA_STRUCT_javax_rtcbench_Point *)REF_TO_VOIDP(MoteTrack_DB_dest_refsignature->location);
    dest_location->x = refSig.location.x;
    dest_location->y = refSig.location.y;
    dest_location->z = refSig.location.z;

    BM_MOTETRACK_NCA_STRUCT_javax_rtcbench_Signature * dest_sig = (BM_MOTETRACK_NCA_STRUCT_javax_rtcbench_Signature *)REF_TO_VOIDP(MoteTrack_DB_dest_refsignature->sig);
    ...
    for (uint8_t i=0; i<NBR_RFSIGNALS_IN_SIGNATURE; i++) {
        BM_MOTETRACK_NCA_STRUCT_javax_rtcbench_RFSignal * dest_rfsignal = (BM_MOTETRACK_NCA_STRUCT_javax_rtcbench_RFSignal *)REF_TO_VOIDP(dest_rfsignals[i]);
...
void javax_rtcbench_DB_void_refSignature_get_javax_rtcbench_RefSignature_short() {
    ...
    MoteTrack_DB_dest_refsignature = (BM_MOTETRACK_NCA_STRUCT_javax_rtcbench_RefSignature *)REF_TO_VOIDP(dj_exec_stackPopRef());
