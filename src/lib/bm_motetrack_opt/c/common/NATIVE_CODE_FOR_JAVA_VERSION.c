...
// 	BM_MOTETRACK_OPT_STRUCT_javax_rtcbench_NewSignature * dest_newsignature = (BM_MOTETRACK_OPT_STRUCT_javax_rtcbench_NewSignature *)REF_TO_VOIDP(dj_exec_stackPopRef());
...
void javax_rtcbench_DB_void_refSignature_get_javax_rtcbench_RefSignature_short() {
	int16_t index = dj_exec_stackPopShort();
	BM_MOTETRACK_OPT_STRUCT_javax_rtcbench_RefSignature * dest_refsignature = (BM_MOTETRACK_OPT_STRUCT_javax_rtcbench_RefSignature *)REF_TO_VOIDP(dj_exec_stackPopRef());

	RefSignature refSig;
	RefSignatureDB_get_JVM(&refSig, index);

	BM_MOTETRACK_OPT_STRUCT_javax_rtcbench_Point * dest_location = (BM_MOTETRACK_OPT_STRUCT_javax_rtcbench_Point *)REF_TO_VOIDP(dest_refsignature->location);
	dest_location->x = refSig.location.x;
	dest_location->y = refSig.location.y;
	dest_location->z = refSig.location.z;

	BM_MOTETRACK_OPT_STRUCT_javax_rtcbench_Signature * dest_sig = (BM_MOTETRACK_OPT_STRUCT_javax_rtcbench_Signature *)REF_TO_VOIDP(dest_refsignature->sig);
