...
void printRFSignalAvgHT(RFSignalAvgHT *h) {
    ...
    for (int i=0; i<h->capacity; i++) {
        ...
        avroraPrintInt16(h->htData[i].rssiSum[0]);
        avroraPrintInt16(h->htData[i].nbrSamples[0]);
        avroraPrintInt16(h->htData[i].rssiSum[1]);
        avroraPrintInt16(h->htData[i].nbrSamples[1]);
    }
    asm volatile ("break");
}
