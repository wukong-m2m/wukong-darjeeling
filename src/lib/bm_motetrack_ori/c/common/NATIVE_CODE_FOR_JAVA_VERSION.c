...
BM_MOTETRACK_ORI_STRUCT_javax_rtcbench_RefSignature * MoteTrack_DB_dest_refsignature;
...
void read_refsignature_from_flash() {
    ...
    BM_MOTETRACK_ORI_STRUCT_javax_rtcbench_Point * dest_location = (BM_MOTETRACK_ORI_STRUCT_javax_rtcbench_Point *)REF_TO_VOIDP(MoteTrack_DB_dest_refsignature->location);
    dest_location->x = refSig.location.x;
    dest_location->y = refSig.location.y;
    dest_location->z = refSig.location.z;

    BM_MOTETRACK_ORI_STRUCT_javax_rtcbench_Signature * dest_sig = (BM_MOTETRACK_ORI_STRUCT_javax_rtcbench_Signature *)REF_TO_VOIDP(MoteTrack_DB_dest_refsignature->sig);
    ...
    for (uint8_t i=0; i<NBR_RFSIGNALS_IN_SIGNATURE; i++) {
        BM_MOTETRACK_ORI_STRUCT_javax_rtcbench_RFSignal * dest_rfsignal = (BM_MOTETRACK_ORI_STRUCT_javax_rtcbench_RFSignal *)REF_TO_VOIDP(dest_rfsignals[i]);
...
void javax_rtcbench_DB_void_refSignature_get_javax_rtcbench_RefSignature_short() {
    ...
    MoteTrack_DB_dest_refsignature = (BM_MOTETRACK_ORI_STRUCT_javax_rtcbench_RefSignature *)REF_TO_VOIDP(dj_exec_stackPopRef());
    read_refsignature_from_flash();
}
void javax_rtcbench_DB_void_signature_get_javax_rtcbench_RefSignature_short() {
    MoteTrack_DB_index = dj_exec_stackPopShort();
    MoteTrack_DB_dest_refsignature = (BM_MOTETRACK_ORI_STRUCT_javax_rtcbench_RefSignature *)REF_TO_VOIDP(dj_exec_stackPopRef());
    read_signature_from_flash();
}
#endif
