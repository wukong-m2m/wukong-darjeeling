...
void printSignature(Signature *s) {
    ...
    for (int i=0; i<NBR_RFSIGNALS_IN_SIGNATURE; i++) {
        ...
        avroraPrintInt16(s->rfSignals[i].rssi[0][0]);
        avroraPrintInt16(s->rfSignals[i].rssi[1][0]);
    }
    asm volatile ("break");
}
...
void printRefSignature(RefSignature *r) {
    ...
    for (int i=0; i<NBR_RFSIGNALS_IN_SIGNATURE; i++) {
        ...
        avroraPrintInt16(r->sig.rfSignals[i].rssi[0][0]);
        avroraPrintInt16(r->sig.rfSignals[i].rssi[1][0]);
    }
    asm volatile ("break");
}
