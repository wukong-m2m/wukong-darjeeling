...
void __attribute__((noinline)) rtcbenchmark_measure_native_performance(uint16_t NUMNUMBERS, int16_t buffer[], int16_t distance_threshold, bool outliers[]) {
	rtc_startBenchmarkMeasurement_Native();

	for (uint16_t i=0; i<NUMNUMBERS; i++) {
		uint16_t exceed_threshold_count = 0;
		for (uint16_t j=0; j<NUMNUMBERS; j++) {
			int16_t diff = buffer[i] - buffer[j];
