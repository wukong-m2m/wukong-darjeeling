    ...
    switch (opcode) {
        ...
        case JVM_GETCONSTARRAY_I:
            ...
            if (array_header_size > 0) { // if array_header_size > 0 we are reading from a normal array
                ...
                // Ask the stack cache for another available register pair, but it shouldn't spill to memory if nothing is available.
        ...
        case JVM_STOP_AOT_BM:
            rtc_flush_and_cleartags_ref(RTC_FILTER_ALL, RTC_FILTER_ALL); // To prevent rtc_stopBenchmarkMeasurement from corrupting the return value.
