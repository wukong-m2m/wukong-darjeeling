...
void wuclass_equal_update(wuobject_t *wuobject) {
  ...
  wkpf_internal_read_property_int16(wuobject, WKPF_PROPERTY_EQUAL_INPUT2, &input2);

  output = (input1 == input2) ? true : false;
  wkpf_internal_write_property_boolean(wuobject, WKPF_PROPERTY_EQUAL_OUTPUT, output);
