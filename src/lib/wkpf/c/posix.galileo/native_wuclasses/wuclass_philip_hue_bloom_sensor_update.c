...
void wuclass_philip_hue_bloom_sensor_update(wuobject_t *wuobject)
{
    ...
    if (currenttime - lasttime > loop_rate){
        ...
        wkpf_internal_write_property_boolean(wuobject, WKPF_PROPERTY_PHILIP_HUE_BLOOM_SENSOR_ON, on); 
        DEBUG_LOG(DBG_WKPFUPDATE, "WKPFUPDATE(Philip_Hue_Bloom_Sensor): red: %d\n", r);
        DEBUG_LOG(DBG_WKPFUPDATE, "WKPFUPDATE(Philip_Hue_Bloom_Sensor): green: %d\n", g);
        DEBUG_LOG(DBG_WKPFUPDATE, "WKPFUPDATE(Philip_Hue_Bloom_Sensor): blue: %d\n", b);
        DEBUG_LOG(DBG_WKPFUPDATE, "WKPFUPDATE(Philip_Hue_Bloom_Sensor): on: %s\n", (on)?"true":"false");
        lasttime = currenttime;
    }
}
