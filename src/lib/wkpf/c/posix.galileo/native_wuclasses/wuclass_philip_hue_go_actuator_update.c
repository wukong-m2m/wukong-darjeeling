...
void wuclass_philip_hue_go_actuator_update(wuobject_t *wuobject)
{
    ...
    if (currenttime - lasttime > loop_rate){
        if (hga_gamma < 0){
            hga_gamma = get_gamma(ip, message, MESSAGE_SIZE, str, NULL, NULL, NULL, NULL);
            if (hga_gamma < 0) {
        ...
        if (!ret){
            DEBUG_LOG(DBG_WKPFUPDATE, "WKPFUPDATE(Philip_Hue_Go): Setting red to: %d\n", r);
            DEBUG_LOG(DBG_WKPFUPDATE, "WKPFUPDATE(Philip_Hue_Go): Setting green to: %d\n", g);
            DEBUG_LOG(DBG_WKPFUPDATE, "WKPFUPDATE(Philip_Hue_Go): Setting blue to: %d\n", b);   
            DEBUG_LOG(DBG_WKPFUPDATE, "WKPFUPDATE(Philip_Hue_Go): Setting on_off to: %x\n", on);
        } else {
            DEBUG_LOG(DBG_WKPFUPDATE, "WKPFUPDATE(Philip_Hue_Go): Error! %d\n", ret);
        }
        lasttime = currenttime;
    }
}
