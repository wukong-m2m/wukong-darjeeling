...
bool wkreprog_open_file(uint8_t filenumber, uint16_t start_write_position) {
	...
	if (dj_archive_number_of_files(di_app_archive) <= filenumber
		...
		return false;
	return wkreprog_impl_open_app_archive(file+start_write_position - di_app_archive);
}
