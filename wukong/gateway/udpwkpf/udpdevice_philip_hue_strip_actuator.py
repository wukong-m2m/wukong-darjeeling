...
if __name__ == "__main__":
    ...
    class Philip_Hue_Strip_Actuator(WuClass):
        def __init__(self):
            WuClass.__init__(self)
            self.loadClass('Philip_Hue_Strip_Actuator')
            self.lasttime = int(round(time.time() * 1000))
            self.loop_rate = 500
            self.gamma = Gamma()
            print "Hue Strip Actuator init success"

        def update(self,obj,pID=None,val=None):
            debug_name = "Philip_Hue_Strip_Actuator"
            user = "newdeveloper"
    ...
    class MyDevice(Device):
        ...
        def init(self):
            self.m = Philip_Hue_Strip_Actuator()
