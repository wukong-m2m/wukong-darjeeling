...
class Testrtt(tornado.web.RequestHandler):
    def get(self):
        modelmanager.refreshNodeInfo()
        testrtt = template.Loader(os.getcwd()).load('templates/testrtt.html').generate(log=['Please press the buttons to add/remove nodes.'], node_infos=modelmanager.getNodeInfos(), set_location=True, default_location = LOCATION_ROOT)
        self.content_type = 'application/json'
